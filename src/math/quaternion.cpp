...
namespace dogEngine
{
	...
	CQuaternion::CQuaternion(float x0,float x1,float x2,float x3)
	{
	...
	*/
	CQuaternion::CQuaternion(CVector3 rotationAxis,float angle)//angle in radian
	...
	CQuaternion CQuaternion::operator +(CQuaternion sum)
	{
		CQuaternion ret;
	...
	*/
	CVector3 CQuaternion::operator *(CVector3 vec)
	{
		...
		CQuaternion vecQuad = CQuaternion(0.0f,vec.getX(),vec.getY(),vec.getZ());
		CQuaternion result = vecQuad * conjugate();
		result = (*this) * result;
		CVector3 ret = CVector3(result.getX(), result.getY(), result.getZ());
		ret *= length;
		return ret;
	}
	...
	{
		return CQuaternion(this->components[0],-this->components[1],-this->components[2],-this->components[3]);
	}

	CQuaternion CQuaternion::inverse()
	{
		CQuaternion conju = this->conjugate();
		CQuaternion erg = (*this) * conju;
		erg = erg.conjugate();
		return (*this) * erg;
	}
	/*
...
Matrix4 Quaternion::getMatrix() const
{
	 ...
	 */
	CMatrix4 CQuaternion::getMatrix()
	{
		float x2 = getX() * getX();
		float y2 = getY() * getY();
		float z2 = getZ() * getZ();
	...
	*/
	void CQuaternion::getAxisAngle(CVector3& axis,float& angle)
	{
	...
	void CQuaternion::setW(float w)
	{
		this->components[0] = w;
	}
	void CQuaternion::setX(float x)
	{
		this->components[1] = x;
	}
	void CQuaternion::setY(float y)
	{
		this->components[2] = y;
	}
	void CQuaternion::setZ(float z)
	{
		this->components[3] = z;
	}
}
